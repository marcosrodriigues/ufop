...
public class Servidor2 {
	...
	public static void main(String[] args) throws Exception {
		...
		Globals.outroServidor = "10.128.0.2";
		serverPrincipal = false;
		new Servidor2().executa();
	}

	public Servidor2() {
		Servidor2.clientes = new ArrayList<String>();
	}

	public void executa() throws IOException {
		Servidor2.servidor = new ServerSocket(Globals.PORT);
		...
		while (true) {
			try {
				if (serverPrincipal) {
					...
					if (desatualizado && ping.isReachable(Globals.delay/50)) {
						try {
					...
					if (filaAlteracoes.size() > 0) {
						...
						try {
							Globals.sendMessage(ipAtual, Globals.outroServidor, Globals.SENDCHANGE, Globals.PORT,
									mudancas);
						} catch (Exception e) {
