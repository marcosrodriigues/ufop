...
void Search_BStar(TRegistro *x, TipoApontadorEstrela *Ap, int key, int *nTransfer, int *nCompare) {
	...
	int i;
  	...
  	if ((*Ap)->PT == INTERNA) {
    	...
    	cout << "Chave procurada: " << key << endl;
    	cout << "Chave atual: " << (*Ap)->UU.U0.ri[i - 1].chave << endl;
    	cout << "Chave à direita: " << Pag->UU.U0.ri[i].chave <<  endl;    	
	...
	cout << "Chave que existe: " << Pag->UU.U1.re[i - 1].chave << endl;
...
void Ins_Star (TRegistro reg, TipoApontadorEstrela Ap, short *Cresceu, TRegistro *RegRetorno, TipoApontadorEstrela *ApRetorno) {
	...
	if (i < M + 1) {
		...
		cout << "Registro " << RegRetorno->chave << " adicionada na página dividida." << endl;
		Ap->UU.U0.ni--;
		InsertInPage_Star(Ap, *RegRetorno, *ApRetorno);
...
void escreverValor(TipoApontadorEstrela *Ap) {
	...
	if ((*Ap)->PT == INTERNA) {
		for (int i = 0; i < (*Ap)->UU.U0.ni; ++i) {
			cout << "Codigo: " << (*Ap)->UU.U0.ri[i].chave << endl;
		}
	} else {
		for (int i = 0; i < (*Ap)->UU.U1.ne; ++i) {
			cout << "Codigo: " << (*Ap)->UU.U1.re[i].chave << endl;
		}
	}
}
